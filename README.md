...
![战斗画面](screenshots/02-gameplay.jpg)

## 🎯 这是个什么游戏

**一句话：在越来越厚的尸潮里活下去。**

- **核心循环**：一波僵尸从屏幕外涌来 → 清光这一波 → 从四张强化卡里选一张 → 下一波更厚更硬。
  第 5、10、15… 波会投放 **Boss**（5 只循环轮换），打掉有掉落雨。
- **成长是乘算的，敌人的成长也是配套设计的**：开局一把特斯拉电枪，靠武器箱换枪（8 把、三档强度）
  再叠强化卡，输出能翻近十倍；僵尸血量则按波次**复利**上涨（第 30 波是第 1 波的 20 倍）。
  两条曲线是配着调的 —— 所以后期既不会「随便秒杀」，也不会「打不动」。
  实测：加特林杀一只杂兵，第 1 波 0.12 秒、第 30 波 0.77 秒（旧版是 0.12 → 0.23 秒，曲线是平的）。
- **每局都不一样**：12 种僵尸按波次逐步解锁（每种都带一个逼你改变打法的机制）、
  武器掉落随机、出怪方向与节奏随机、地面由预制区块随机拼接。
- **几分钟一把，随时能退**：暂停菜单里可以直接「结束本局」，成绩照记；打死了进结算，
  会显示你在这台机器**排行榜**上的名次并高亮那一行。
- **能带一只宠物**：5 只宠物跟着你自动攻击、靠击杀升级；VIP 的宠物上限更高、每 5 级还会进化。

**零依赖、零构建、双击 `index.html` 即玩，完全离线。**

## ✨ 特性一览

- 🧟 **12 种特殊僵尸**：不只是体型差异 —— 远程吐酸、贴身自爆、子弹减伤的装甲者、**狂化尸群的尖啸者**、**死亡会裂成两只的裂殖体**、**假死一次的复生者**、**治疗同伴的孢囊**、**带落点预警圈的跃行者**……各有专属机制与造型
...
- 🧭 **屏幕外的方向指示标**：屏幕外还剩几只僵尸，就在屏幕边缘对应方向画一个箭头（Boss 是橙色）——
  不会再出现「明明还剩一只却找不到」的干等（屏幕外的僵尸也会走快一点进场）
- 🗺️ **随机拼接的战场**：地面由预制区块（沥青 / 水泥板 / 裸土 / 积水 / 碎屑）随机拼接，
  接缝只沿一个「浇筑方向」，雨夜里还有几处灯池反光与积水镜面
...
![开始界面](screenshots/01-start.jpg)

> 开始界面一屏搞定：左边普通模式、右边 VIP 模式，下面是宠物栏（VIP 那两只带锁标，
> 鼠标移到 VIP 卡上会解锁）。左上角是排行榜入口。
...
![排行榜](screenshots/07-board.jpg)

![结算界面](screenshots/08-over.jpg)

> 上面是排行榜面板，下面是结算界面（显示本局排名 + 把刚打完的那一条高亮 + 提供「再来一局 / 返回主界面」）。
...
| 重装巨兽 | 第 4 波 | 巨型近战肉盾 |
| **跃行者** | 第 5 波 | **会主动走近**，进射程后蹲伏蓄力（地面画出**落点圈 + 虚线指向**），随后一扑一大段、落地僵直 0.5 秒 —— 走出圈再回来打 |
| **复生者** | 第 6 波 | 第一次倒下只是**假死**：半血重新站起来，必须杀两次 |
| **裂殖体** | 第 6 波 | 死亡时**裂成两只残体** —— 杀它反而让场上变多，想清楚是该放着还是用范围武器一起清 |
| **孢囊** | 第 7 波 | 周期性**治疗周围尸群**（自己回一点）—— 不先处理它，整波都会变厚 |
| 残体 | 仅分裂产生 | 裂殖体的碎片：小、快、脆，**不进生成表**（只由裂殖体死亡时产生） |

![十二种僵尸](screenshots/06-zombies.jpg)

> 上图是 12 种杂兵按游戏内朝向放大的样子（剪影、体型与颜色都刻意拉开距离，
> 小尺寸下也能一眼分辨该躲谁、该先打谁）。
...
> **出场位置与节奏（刻意做随机）**：落点是「从玩家出发按**均匀随机的角度**走出屏幕，
> 再多走 40~160px 的随机距离」，**不是**四条边等概率。旧写法里左右两边离玩家更远、角跨度更窄，
> 同样是 25% 的概率被压在更小的角度里 —— 实测东西方向来怪的密度是上下的 **1.45 倍**
> （方向直方图峰值/理想 **1.53×**），玩几局就会条件反射地把枪口对着左右。按角度取样后
> 峰值/理想 ≈ **1.05×**，Boss 出场点同理（1.6× → 1.1×）。
> 出怪节奏也加了随机：间隔乘 `rand(0.55, 1.5)` 抖动，并且偶发「一涌而上」补 1~2 只；
> 额外只数会让总量变多，所以触发间隔按期望只数拉长（`SPAWN_BURST_COMP`），
> **平均出怪速度保持 ≈1.95 只/秒 不变** —— 只加随机性，不偷偷改难度。
> 旋钮都在 `js/data.js`：`SPAWN_MARGIN_MIN` / `SPAWN_MARGIN_VAR` / `SPAWN_BURST_P` / `SPAWN_BURST_COMP`。

> **种类是怎么排进出场表的**：`SPAWN_TABLE` 是「[类型, 累计阈值]」的有序表，未到 `minWave` 的类型被跳过、
> 其概率质量归给普通感染者。新增的四种排在 `runner` **之后** —— 也就是从「普通感染者的尾段」里拿份额，
> 所以在它们各自 `minWave` 之前的波次里，生成分布与扩充前**逐项一致**（第 1 波仍 100% 普通、
> 第 2 波 76% 疾跑者、第 3 波 42% 自爆者），**不会偷偷改前期难度**；回归里用固定期望值钉住了这一点。
> 想再调各族的占比，只改这张表的阈值即可（残体不在此表，只由裂殖体分裂产生）。
...
![Boss 战](screenshots/03-boss.jpg)

> 上图同时能看到三种预警表现：冲撞者蓄力时地上的**橙色指向线**、迫击者炮弹的**落点警示圈**（内圈随引信收缩）、
> 以及纳尸者身边的**尸体堆**（它会把这些尸体重新拉起来）。
...
![强化卡](screenshots/04-cards.jpg)

> 每波肃清后四选一。卡面直接写「当前 → 选后」的真实数值，不用心算百分比；不适用的卡不会出现。
...
![宠物栏](screenshots/05-pets.jpg)

> 开始界面的宠物栏：一屏常驻，VIP 专属那两只有 VIP 标与锁标；鼠标移到模式卡上会切换可选项。
...
├── docs/
│   ├── 美术与手感升级方案.md   # 角色造型 / 打击感的升级路线与验收标准（P0 已落地）
│   └── 地图视觉进化方案.md     # 战场地面的诊断 / 技术路线 / 可测验收（待排期）
├── css/
│   └── style.css       # 界面样式
├── js/
