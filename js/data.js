...
// LEAP_RANGE 是「进这个距离就蓄力扑击」的触发范围 —— 注意**待机时期它照常追击**，
// 所以远处也会主动走过来，而不是站着等玩家靠近。
const LEAP_SPEED = 660;
const LEAP_TIME = 0.32;
const LEAP_RANGE = 460;
// 屏幕外的"走进场"加速倍率。实测：不加速时单只僵尸最慢要 10–14 秒才贴到玩家身上
// （出生距离最远 880px、最慢的孢囊只有 34px/s），而波次要等最后一只到场才能清 ——
// 那十几秒玩家什么都没得打，看起来就像"卡住了"。加速只作用于屏幕外、进屏立刻恢复，
// 所以对真正的战斗难度零影响（难度只发生在你看得见的地方）。
const WALKIN_MUL = 2.6;
// 地形：预制区块边长 + 确定性种子。种子固定 ⇒ 同一尺寸下地形可复现（重建不会变样、
// 视觉回归可对照）；以后想"每局换一张地图"把种子改成每局随机即可。
const CHUNK_SIZE = 300;
const TERRAIN_SEED = 0x5eed1234;
...
const PAL = {
  ...
  // ===== 地形区块（雨夜·城郊废弃停车场）=====
  // 旧版是「随机椭圆给出明暗」：半透明大椭圆叠加时边界会显影，眼睛能追出来 → 读成"泡泡/油渍"。
  // 现在改成「预制区块 + 确定性噪声」：用途决定基色，噪声决定材质，接缝给出方向。
  asphalt:     '#1d2019',                  // 湿沥青：比基底更冷更暗
  slab:        '#2a2d27',                  // 水泥板：最亮的一档（结构感的来源）—— 场上主体
  dirt:        '#2b2617',                  // 裸土 / 淤泥（不能太棕太亮：大块棕会盖掉整场的结构）
  rubbleCol:   '#2e2a22',                  // 碎屑堆
  puddleCol:   'rgba(104,152,214,0.26)',   // 积水：必须够冷够实，否则在直方图里根本不算"冷色"
  puddleHi:    'rgba(226,240,255,0.22)',   // 积水柔光
  puddleCore:  'rgba(232,244,255,0.85)',   // 积水镜面芯：地面唯一能顶到高光区的东西（"湿"的全部说辞）
  jointLight:  'rgba(198,212,190,0.12)',   // 接缝亮边
  jointDark:   'rgba(0,0,0,0.44)',         // 接缝暗芯（环境遮蔽的一半就在这条暗芯上）
  laneMark:    'rgba(214,206,160,0.10)',   // 褪色车道线（人造痕迹，给出尺度）
